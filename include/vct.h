...
#define VCT_IMPL
	...
	void* vct_calloc(vct* v, size_t count, size_t sz) {
		if (!v->allocators.initialized) return NULL;      // ensure allocator is set
		void* ptr = v->allocators.malloc(count * sz);     // allocate memory
	...
	{
		if (v->allocators.initialized) return;
		vct_memcpy(&v->allocators, allocators, sizeof(vct_allocators));
		v->allocators.initialized = 1u;
	}
	...
	{
		if (!v) return;
		if (v->data) v->allocators.free(v->data);
	}
	...
	int vct_push_any(vct* v, const void* data, size_t size)
	{
		...
		if (new_size > v->capacity)
		{
			...
			void* new_data = v->allocators.realloc(v->data, new_capacity * v->stride);
